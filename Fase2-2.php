...
use Curl\Curl;
    ...
    while ($i > 2){
        ...
        $Consulta1 = $Core->MySQL->query("SELECT * FROM Operaciones WHERE Estado='0' AND Ingresada='1' AND Ob1_A='0' ORDER BY RAND() LIMIT 0,1");
        ...
        if (empty($Core->MySQL->error)){
            ...
            if ($Consulta1->num_rows == 1){
                ...
                if ($Ticker->Precio >= $Objetivo_1){

                    $Core->MySQL->query("UPDATE Operaciones SET Ob1_A='1', Stop='$Precio_Compra' WHERE id='$oid'");
